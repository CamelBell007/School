...
public class DeviceTypeRequest extends BaseRequest {
	...
	public DeviceTypeRequest(String treeGradeId) {
	...
	public String getUrl() {
		return Constants.BASE_URL+Constants.REQUEST_NAME.DEVICE_TYPE_LIST;
	}
	 ...
	 * @return
	 */
	public ResultInfo<DeviceTypeInfo> getDeviceTypeInfo() {
		ResultInfo<DeviceTypeInfo> result = new ResultInfo<DeviceTypeInfo>();
		...
		for (int i = 0; i < data.length(); i++) {
			...
			DeviceTypeInfo info = new DeviceTypeInfo(jo);
			if (info != null) {
				list.add(info);
			}
		}
