package cn.yoho.library.widget.gif;
...
public class GifView extends View implements GifAction {
    ...
    /**
     * 手动设置动画快慢（每一帧之间的  延时 时间）
     * 小于等于零 则使用默认时间（根据解码时候获取的默认延时时间）
     */
    public void SetDELAY(long delay)
    {
        if(delay<=0)
        {
            DELAY=0;//设置为零，即为默认延时时间
            return;
        }
        DELAY=delay;
    }
     ...
     * @param gif 要设置的图片
     */
    public void setSource(byte[] gif){
        if(gifDecoder != null){
            gifDecoder.free();
            gifDecoder = null;
        }
    ...
    /**
     * 以资源形式设置gif图片(资源转换为 数据流 设置gif图片)
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(rectOut == null){
            ...
            if(showWidth == -1){
                ...
                canvas.drawBitmap(currentImage, null, rectOut, null);
            }
        }
